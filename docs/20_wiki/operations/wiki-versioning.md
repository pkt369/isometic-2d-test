...
1. 변경 범위를 확인한다.
2. GitLab Release 최신 tag와 `index.md`의 `version`을 비교한다. 불일치 시 [[wiki-sync]]의 "원격 버전 동기화"를 따른다.
...
8. GitLab Release를 만든다. 제목은 tag와 같게, 본문에는 해당 날짜 log의 버전 변경 요약을 압축해 적는다.
