...
- **개별 프로젝트에서만 실행한다.** frontend-wiki 저장소에서는 실행하지 않는다.
- 프로젝트 셋팅 시 [[agent-instruction-guide]] 부트스트랩과 함께 설치·gitignore를 확인한다.
...
- 프로젝트 `docs/`는 [[project-wiki-guide]] 구조(`raw/`·`worklog.md`)를 따르므로, superpowers 산출물이 그 구조에 섞여 커밋되지 않게 여기서 막는다.
