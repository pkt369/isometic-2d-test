...
audit_log:
  ...
  - action: updated
    at: 2026-07-02
    by: 정회석
    note: "역할 분리 표 backend-wiki 오타를 frontend-wiki로 정정"
...
2. 조회 결과는 위키에 저장하지 않는다. 저장할 가치가 있는 건 문서 자체가 아니라 **그 기술을 쓰기로 한 판단**이며, 그건 [[expo-sdk-54-pinning]]처럼 `/20_wiki/principles/`에 남긴다.
